...
import co.skliba.rideme.BuildConfig
import co.skliba.rideme.data.network.ApiService
...
class ApiModule {
    ...
    internal fun okHttpClient(context: Context): OkHttpClient {
        ...
        if (BuildConfig.DEBUG) {
            val logger: HttpLoggingInterceptor.Logger = HttpLoggingInterceptor.Logger { Timber.tag("OkHttp").w(it) }
            okHttpBuilder.addInterceptor(HttpLoggingInterceptor(logger).apply { level = HttpLoggingInterceptor.Level.BODY })
        } else {
